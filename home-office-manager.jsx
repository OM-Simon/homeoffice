...
const TEAM = [
  { id: 1,  name: "Ana Silva",      avatar: "AS", color: "#6366f1" },
  { id: 2,  name: "Bruno Costa",    avatar: "BC", color: "#f59e0b" },
  { id: 3,  name: "Carla Mendes",   avatar: "CM", color: "#10b981" },
  { id: 4,  name: "David Rocha",    avatar: "DR", color: "#ef4444" },
  { id: 5,  name: "Eva Pinto",      avatar: "EP", color: "#8b5cf6" },
  { id: 6,  name: "Filipe Antunes", avatar: "FA", color: "#06b6d4" },
  { id: 7,  name: "Gonçalo Neves",  avatar: "GN", color: "#f43f5e" },
  { id: 8,  name: "Helena Sousa",   avatar: "HS", color: "#84cc16" },
  { id: 9,  name: "Igor Ferreira",  avatar: "IF", color: "#fb923c" },
  { id: 10, name: "Joana Matos",    avatar: "JM", color: "#a78bfa" },
  { id: 11, name: "Kevin Lopes",    avatar: "KL", color: "#2dd4bf" },
  { id: 12, name: "Lara Fonseca",   avatar: "LF", color: "#fb7185" },
  { id: 13, name: "Miguel Saraiva", avatar: "MS", color: "#facc15" },
];
...
export default function HomeOfficeApp() {
  ...
  const [view, setView] = useState("calendar"); // calendar | team
  ...
  const getKey = (year, month, day) => `${year}-${String(month + 1).padStart(2, "0")}-${String(day).padStart(2, "0")}`;

  const getUserBookingsThisMonth = (userId) => {
  ...
  const toggleBooking = (day) => {
    ...
    if (isBooked) {
      setBookings(prev => ({
        ...prev,
        [key]: prev[key].filter(id => id !== currentUser.id)
      }));
      ...
      if (usedBalance >= MONTHLY_BALANCE) {
        showToast(`Saldo esgotado! Máximo ${MONTHLY_BALANCE} dias/mês.`, "error");
        return;
      }
      if (dayBookings.length >= MAX_PER_DAY) {
        showToast(`Limite atingido! Máximo ${MAX_PER_DAY} pessoas/dia.`, "error");
        return;
      }
      setBookings(prev => ({
        ...prev,
        [key]: [...(prev[key] || []), currentUser.id]
      }));
  ...
  return (
    <div style={{
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {TEAM.map(u => (
            <button key={u.id} onClick={() => setCurrentUser(u)} style={{
              ...
              boxShadow: currentUser.id === u.id ? `0 0 12px ${u.color}60` : "none",
              outline: "none",
      ...
      <div style={{ padding: "0 24px", marginTop: 0, background: "#0f0f13" }}>
        <div style={{ height: 4, background: "#ffffff10", borderRadius: 4, overflow: "hidden", marginTop: 0 }}>
          <div style={{
      ...
      {view === "calendar" ? (
            ...
            {calendarDays.map((day, i) => {
              ...
              return (
                <div key={day} onClick={() => !isWeekend && toggleBooking(day)} style={{
                  ...
                  borderRadius: 10,
                  padding: "8px 6px",
                  background: isBookedByMe
                    ? `${currentUser.color}25`
                    : isToday ? "#ffffff08" : isWeekend ? "transparent" : "#ffffff05",
                  border: isBookedByMe
                    ? `1.5px solid ${currentUser.color}60`
                    : isToday ? "1.5px solid #ffffff20" : "1.5px solid transparent",
        ...
        <div style={{ padding: "16px 24px 32px" }}>
              ...
              return (
                <div key={u.id} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                    ...
                    <div style={{
                      fontSize: 20, fontWeight: 800, color: rem <= 2 ? "#ef4444" : u.color,
                    }}>{rem}</div>
                  </div>
                  <div style={{ height: 6, background: "#ffffff10", borderRadius: 4, overflow: "hidden" }}>
                    <div style={{
                      ...
                      background: rem <= 2
                        ? "linear-gradient(90deg, #ef4444, #f87171)"
                        : `linear-gradient(90deg, ${u.color}, ${u.color}88)`,
          ...
          <div style={{ marginTop: 24 }}>
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12, color: "#9ca3af" }}>
              Hoje em HO
            </div>
            {(() => {
              const todayKey = getKey(currentYear, currentMonth, today.getDate());
              const todayUsers = (bookings[todayKey] || []).map(id => TEAM.find(u => u.id === id)).filter(Boolean);
